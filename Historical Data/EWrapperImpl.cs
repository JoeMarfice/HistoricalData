...
namespace IB_Console
{
    ...
    //! [ewrapperimpl]
        ...
        public MyCode.AllPeriodsClass Handler;

        //! [socket_init]
        public EWrapperImpl(string StockSymbolParam)
        {
            StockSymbol = StockSymbolParam;
            Handler = new MyCode.AllPeriodsClass(StockSymbol);
        ...
        ) 
        {
            if (field == 4)
            {
                Handler.HandleTickOrder(DateTime.Now, price, StockSymbol);
                //Handler.HandleTickOrder(DateTime.Now, price+100, StockSymbol);
                /*
                strOutput =
                    "Tick Price. Ticker Id:" + tickerId +
                    ", Field: " + field +
                    ", Price: " + price +
                ...
                */
                /*
                double dblMovingAverage;
                string strOutput;
                IB_Console.Program.dblPriceTotal += price;
                dblMovingAverage = IB_Console.Program.dblPriceTotal 
                    / IB_Console.Program.iNumberOfPrices;
                strOutput += ", Avg Trade Price: " + String.Format("0.00", dblMovingAverage);
